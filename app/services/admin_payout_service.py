    ...
    async def send_payout_request_to_admin(
        ...
        try:
            ...
            admin_message += (
                ...
                f"1. Нажмите кнопку '💳 Оплатить {amount_str} {currency_symbol}'\n"
                f"2. Telegram откроет платежное окно\n"
                f"3. Подтвердите оплату\n"
                f"4. Бот АВТОМАТИЧЕСКИ зачислит {currency_symbol} на баланс победителя в БД\n"
                f"5. Победитель сможет использовать баланс для новых розыгрышей\n\n"
                f"💡 <b>Важно:</b> После оплаты инвойса Stars зачисляются на баланс победителя "
                f"<u>внутри бота</u>, а не напрямую в Telegram Stars.\n\n"
                f"⚠️ Убедитесь, что у вас есть {amount_str} {currency_symbol} на балансе!"
            )
            ...
            keyboard = InlineKeyboardMarkup(inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text=f"💳 Оплатить {amount_str} {currency_symbol}",
                        url=invoice_link
                    )
                ],
                [
                    InlineKeyboardButton(
                        text="✅ Подтвердить (если оплатили вручную)",
